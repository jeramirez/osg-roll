...
class Command(rocks.commands.sync.host.command):
	...
	<example cmd='sync host osg rsv rsv-0-0'>
	Write and run script /root/rsv_iniConfigurator to rewrite inifiles and call 
	osg-configure -c on host rsv-0-0
	</example>
	...
	def run(self, params, args):
		...
		for host in hosts:
			...
			cmd = '/opt/rocks/bin/rocks report host osg rsv config '
			cmd += '%s | ' % host
			...
			if istest:
				...
				cmd += '; '
			cmd += 'ssh %s "/root/rsv_iniConfigurator" ' % host
			if not istest:
				cmd += '> /dev/null 2>&1 '
			...
			if istest:
				...
				cmd += '; '
			cmd += 'ssh %s "/usr/sbin/osg-configure -c" ' % host
			if not istest:
				cmd += '> /dev/null 2>&1 '
