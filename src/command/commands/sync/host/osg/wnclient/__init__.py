...
class Command(rocks.commands.sync.host.command):
	"""
	Reconfigure OSG wnClient on the named hosts.
	...
	<example cmd='sync host osg wnclient compute-0-0'>
	Write and run script /root/wnclientConfigurator on host compute-0-0
	</example>
	...
	def run(self, params, args):
		...
		for host in hosts:
			...
			cmd = '/opt/rocks/bin/rocks report host osg wnclient config '
			cmd += '%s | ' % host
			...
			if istest:
				...
				cmd += '; '
			cmd += 'ssh %s "/root/wnclientConfigurator" ' % host
			if not istest:
				cmd += '> /dev/null 2>&1 '
