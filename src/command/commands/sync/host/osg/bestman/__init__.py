...
class Command(rocks.commands.sync.host.command):
	...
	<example cmd='sync host osg bestman se-0-0'>
	Write and run script /root/BestmanConfigurator to rewrite /etc/bestman2/conf/bestman2.rc and /etc/sysconfig/bestman2 
	on host se-0-0
	</example>
	...
	def run(self, params, args):
		...
		for host in hosts:
			...
			cmd = '/opt/rocks/bin/rocks report host osg bestman config '
			cmd += '%s | ' % host
			...
			if istest:
				...
				cmd += '; '
			cmd += 'ssh %s "/root/BestmanConfigurator" ' % host
			if not istest:
				cmd += '> /dev/null 2>&1 '
