...
class Command(rocks.commands.sync.host.command):
	...
	<example cmd='sync host osg hadoop install node-0-0'>
	...
	<example cmd='sync host osg hadoop install node-0-0 test=yes'>
	...
	def run(self, params, args):
		...
		for host in hosts:
			...
			cmd = '/opt/rocks/bin/rocks report host osg hadoop install '
			cmd += '%s | ' % host
