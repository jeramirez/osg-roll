	...
	def run(self, params, args):
		...
		self.beginOutput()
			...
			if osg_ce > 0:
				...
				self.addOutput(self.host, '/usr/sbin/useradd -u &OSG_tomcatuid; -g &OSG_tomcatgid; -c "Tomcat" -s /bin/sh -d -m /usr/share/tomcat&rocks_version_major; tomcat')
				...
				if osg_export > 0:
					...
					self.addOutput(self.host, '# if needed add %s/ce to /etc/export' % osg_export )
					self.addOutput(self.host, '[ "`grep -c %s/ce /etc/exports`" != "0" ]||echo "%s/ce %s(rw,async,no_root_squash) %s/%s(rw,async)" &gt;&gt; /etc/export ' % (osg_export,osg_export,ksphost,kspn,kspm) )
					self.addOutput(self.host, '# if needed add %s/app to /etc/export' % osg_export )
					self.addOutput(self.host, '[ "`grep -c %s/app /etc/exports`" != "0" ]||echo "%s/app %s(rw,async,no_root_squash) %s/%s(rw,async)" &gt;&gt; /etc/export ' % (osg_export,osg_export,ksphost,kspn,kspm) )
				self.addOutput(self.host, '')
